...
bool ApplicationWindow::loadDocument(const QUrl &url)
{
    ...
    if(!url.isValid()){
        QMessageBox::critical(this, "Ошибка url", "URL документа не рабочий");
        return false;
    }

    document->documentCountChanged(1);

    QTableView* view = nullptr;

    if (auto tabs = document->findChild<QTabWidget*>()) {
        view = qobject_cast<QTableView*>(tabs->currentWidget());
    }
    ...
    auto table = qobject_cast<SimpleTableModel*>(view->model());

    if (!table) {
        QMessageBox::critical(this, "Ошибка", "Нет модели данных");
        return false;
    }

    if(!importTableToCsv(table, url.toLocalFile())){
...
bool ApplicationWindow::saveDocument(DocumentWidget *document, const QUrl &altUrl)
{
    ...
    if(!url.isValid()){
        QMessageBox::critical(this, tr("Ошибка url"), tr("URL документа не рабочий"));
        return false;
    }
    ...
    if (!view) {
        QMessageBox::critical(this, tr("Ошибка таблицы"), tr("Нет данных для сохранения"));
        return false;
    }
    ...
    if (!table) {
        QMessageBox::critical(this, tr("Ошибка таблицы"), tr("Нет модели данных"));
        return false;
    }
...
bool ApplicationWindow::importTableToCsv(SimpleTableModel *model, const QString &filename)
{
    QFile file(filename);
    ...
    QVector<QVector<QVariant>> newCells;
...
//    int rows = newCells.size();
//    int cols = newCells[0].size();

    model->setAllData(newCells);

    return true;
}


bool ApplicationWindow::exportTableToCsv(SimpleTableModel *model, const QString &filename)
{
    QFile file(filename);
    ...
    for (int r = 0; r < rows; ++r) {
        ...
        for (int c = 0; c < cols; ++c) {
            ...
            row << model->data(idx, Qt::DisplayRole).toString();
        }
        out << row.join(",") << "\n";
    }
...
{
    const QList<QUrl> urls = QFileDialog::getOpenFileUrls(this, tr("Open Documents"), QUrl("C:\\Users\\Danik\\Desktop\\STUDY_SIBSUTIS_DOROGIN\\Visual.p2\\Prakt5\\docs"), "*.csv");
...
void ApplicationWindow::slotSaveAs()
{
    ...
    const QUrl &url = QFileDialog::getSaveFileUrl(this, tr("Save Document"), QUrl("C:\\Users\\Danik\\Desktop\\STUDY_SIBSUTIS_DOROGIN\\Visual.p2\\Prakt5\\docs"), "*.csv");
    if (!url.isEmpty()) {
        document->setUrl(url);
        saveDocument(document, QUrl());
    }
